...
export class ExcelExportService {
  ...
  async exportGstr3bToExcel(report: Gstr3bResponseDto): Promise<Buffer> {
    ...
    // Sheet 2: Output Tax
    const outputTaxSheet = workbook.addWorksheet('Output Tax');
    outputTaxSheet.columns = [
      { header: 'Tax Rate (%)', key: 'rate', width: 12 },
      { header: 'Taxable Value', key: 'taxable_value', width: 15 },
      { header: 'IGST', key: 'igst', width: 12 },
    ...
    outputTaxSheet.addRow({
      rate: 'Total',
      taxable_value: report.output_tax.total_taxable_value,
      igst: report.output_tax.total_igst,
      cgst: report.output_tax.total_cgst,
      sgst: report.output_tax.total_sgst,
      cess: report.output_tax.total_cess,
    });

    const totalRow = outputTaxSheet.getRow(outputTaxSheet.rowCount);
    ...
    // Sheet 3: ITC
    const itcSheet = workbook.addWorksheet('Input Tax Credit');
    itcSheet.columns = [
      { header: 'Description', key: 'description', width: 30 },
      { header: 'Amount', key: 'amount', width: 15 },
    ];

    itcSheet.addRow({ description: 'IGST ITC', amount: report.itc.igst_itc });
    itcSheet.addRow({ description: 'CGST ITC', amount: report.itc.cgst_itc });
    itcSheet.addRow({ description: 'SGST ITC', amount: report.itc.sgst_itc });
    itcSheet.addRow({ description: 'CESS ITC', amount: report.itc.cess_itc });
    itcSheet.addRow({ description: 'Total Eligible ITC', amount: report.itc.total_eligible_itc });
    itcSheet.addRow({ description: 'Total Ineligible ITC', amount: report.itc.total_ineligible_itc });

    const itcTotalRow = itcSheet.getRow(itcSheet.rowCount);
    itcTotalRow.font = { bold: true };
    itcTotalRow.fill = {
      type: 'pattern',
      pattern: 'solid',
