...
import { useHsnValidation } from '@/lib/hooks/use-hsn-validation';
...
export function HsnInput({
  ...
  const { validate, clearValidation, isValid, errors, warnings, isValidating } = useHsnValidation();
  ...
  return (
    <div className={cn('space-y-2', className)}>
      {label && (
        <Label htmlFor="hsn_code" className={required ? "after:content-['*'] after:ml-0.5 after:text-red-500" : ''}>
          {label}
        </Label>
      )}
      ...
      {displayErrors && isValidState && !hasWarning && (
        <p className="text-xs text-green-600">Valid HSN code</p>
      )}
    </div>
  );
}
